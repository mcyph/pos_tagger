...
def get_L_sentences(iso, s):
    with check_nlp_lock:
        if not iso in DCubeInsts:
            assert iso in DSupportedISOs

            # Note the GPU argument - it may be worth putting
            # this on the PC with a more powerful one
            from cube.api import Cube
            cube_inst = DCubeInsts[iso] = Cube(verbose=True)
            # Chinese doesn't seem to work well with version 1.0, despite the scores not showing much difference
            cube_inst.load(DSupportedISOs[iso], version='1.0' if iso == 'zh' else '1.1')

        cube_inst = DCubeInsts[iso]
...
def print_pos(iso, s):
    LSentences = get_L_sentences(iso, s)
