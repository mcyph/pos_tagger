...
class POSTaggerClient(POSTaggersBase,
    ...
    def get_L_sentences(self, iso, s):
        LRtn = self.send(srv.get_L_sentences, [iso, s])
    ...
    def is_alignment_supported(self, from_iso, to_iso):
        return self.send(srv.is_alignment_supported, [from_iso, to_iso])

    def get_aligned_sentences(self,
                              from_iso, to_iso,
                              from_s, to_s):
        L1, L2 = self.send(srv.get_aligned_sentences, [from_iso, to_iso, from_s, to_s])
    ...
    def get_similar_words(self, iso, word, n=30):
        return self.send(srv.get_similar_words, [iso, word, n])

    def get_translations(self, from_iso, to_iso, s):
        return self.send(srv.get_translations, [from_iso, to_iso, s])
